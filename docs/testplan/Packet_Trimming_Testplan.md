...
4. After packet is trimmed, the checksum is not recalculated and IP length is not updated.
...
1. Trimming global config.
   - config: `config switch-trimming global --size 128 --dscp 48 --queue 6`  
...
**Test Steps**
1. Configure trimming via CLI `config switch-trimming global --size 128 --dscp 48 --queue 6`.
...
**Test Steps**
1. Configure packet trimming in global level and set `"size": "128"`.
...
4. Capture egress packets and verify trimmed packet size is 128B.
...
**Test Steps**:  
1. Configure packet trimming in global level and set `"dscp_value": "48"`, `"size": "128"`.  
2. Send 1500B packets with DSCP=0.  
3. Capture egress packets and verify the DSCP value has updated to 48.  
4. Send 100B packets (less than 128) with DSCP=0. 
...
## Test Case 6: Verify ACL Disable Trimming Action
...
## Test Case 7: Configuration Persistence After Reload/Reboot
...
## Test Case 8: Verify Trimming During Multiple Egress Port Admin Toggle
...
## Test Case 9: Stability During Feature State Toggles
...
## Test Case 10: Verify Tail Drop and WRED Functionality When Trimming is Disabled
**Objective**: Validate that Tail Drop and WRED work well when packet trimming is disabled in the buffer profile.  
**Test Steps**:
1. Configure packet trimming in global level and set `"packet_discard_action": "drop"` in buffer profile.
2. ​Verify Tail Drop function, it should work well and not be affected by the trimming function.
3. ​Verify WRED function, it should work well and not be affected by the trimming function.
...
3. Verify the trimming function works well for SRv6 SHIFT packet.
   - Verify the SHIFT SRv6 packet is trimmed, verify the trimmed packet size is correct.
...
4. Verify the trimming function works well for SRv6 DECAP packet.
   - Verify the DECAP SRv6 packet is trimmed, verify the trimmed packet size is correct.
